...
from crypto_futures_bot.constants import BYBIT_FUTURES_TAKER_FEES
...
class BybitFuturesExchangeService(AbstractFuturesExchangeService):
    def __init__(self, configuration_properties: ConfigurationProperties) -> None:
        super().__init__()
        self._configuration_properties = configuration_properties
        if not self._configuration_properties.bybit_api_key or not self._configuration_properties.bybit_api_secret:
            raise ValueError("Bybit API key and secret are required")
        # XXX:For more info about: https://docs.ccxt.com/exchanges/bybit
        commons_options = {
            "apiKey": self._configuration_properties.bybit_api_key,
            "secret": self._configuration_properties.bybit_api_secret,
        ...
        self._spot_client = ccxt.bybit({**commons_options, "options": {"defaultType": "spot"}})
        self._futures_client = ccxt.bybit({**commons_options, "options": {"defaultType": "swap"}})
    ...
    async def get_position_by_id(self, position_id: str) -> Position:
        raise NotImplementedError("Operation not supported in Bybit exchange")
    ...
    async def _get_position_id_by_order_id(self, order_id: str, *, position: Position) -> str:
        ...
        if last_order_status := fetched_order.get("status") != "closed":
            raise ValueError(
                f"Recent order created for {position.symbol} :: {position.position_type}, status is {last_order_status}"  # noqa: 501
            )
        return fetched_order["info"]["positionId"]

    @override
    def get_taker_fee(self) -> float:
        return BYBIT_FUTURES_TAKER_FEES
