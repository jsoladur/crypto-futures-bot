...
class TaskManager:
    def __init__(self, tasks_container: Container) -> None:
        self._tasks_container = tasks_container
        self._tasks: dict[TaskTypeEnum, AbstractTaskService] = {}
    ...
    async def load_tasks(self) -> Self:
        ...
        for global_flag_type in TaskTypeEnum:
            # TODO: Check if global flag type is enabled
            is_global_flag_type_enabled = True
    ...
    async def start(self, global_flag_type: TaskTypeEnum) -> None:
    ...
    async def stop(self, global_flag_type: TaskTypeEnum) -> None:
        if global_flag_type in self._tasks:
    ...
    def _import_task_modules(self) -> dict[TaskTypeEnum, AbstractTaskService]:
        ret: dict[TaskTypeEnum, AbstractTaskService] = {}
