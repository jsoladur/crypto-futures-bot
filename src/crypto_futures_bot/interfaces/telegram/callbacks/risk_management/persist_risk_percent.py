...
REGEX = r"^persist_risk_percent_\$_(.+?)$"
...
async def handle_persist_risk_percent_callback(callback_query: CallbackQuery, state: FSMContext):
    ...
    if is_user_logged:
        try:
            ...
            percent_value = float(match.group(1).strip())
            item: RiskManagementItem = await risk_management_service.get()
            updated_item = replace(item, percent_value=percent_value)
            await risk_management_service.update(updated_item)
            await callback_query.message.answer(
                f"🛡️ Risk Management percent changed to {html.code(str(percent_value))} %",
