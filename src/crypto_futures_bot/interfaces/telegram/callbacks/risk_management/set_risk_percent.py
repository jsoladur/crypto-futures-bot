...
async def set_risk_percent_callback_handler(callback_query: CallbackQuery, state: FSMContext) -> None:
    is_user_logged = await session_storage_service.is_user_logged(state)
    if is_user_logged:
        await callback_query.message.answer(
            "ℹ️ Select the risk percent value to apply", reply_markup=keyboards_builder.get_risk_percent_values()
        )
    else:
        await callback_query.message.answer(
