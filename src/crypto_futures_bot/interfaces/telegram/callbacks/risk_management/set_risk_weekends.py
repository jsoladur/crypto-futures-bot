...
@dp.callback_query(lambda c: c.data == "set_risk_weekends")
async def set_risk_weekends_callback_handler(callback_query: CallbackQuery, state: FSMContext) -> None:
    is_user_logged = await session_storage_service.is_user_logged(state)
    if is_user_logged:
        await callback_query.message.answer(
            "📅 Do you want to allow opening positions on weekends (Saturday/Sunday)?",
            reply_markup=keyboards_builder.get_risk_weekends_values(),
        )
    else:
        await callback_query.message.answer(
