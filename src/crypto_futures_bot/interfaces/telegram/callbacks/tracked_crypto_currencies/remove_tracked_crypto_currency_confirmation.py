...
REGEX = r"^remove_tracked_crypto_currency_\$_(.+)$"


@dp.callback_query(F.data.regexp(REGEX))
async def remove_tracked_crypto_currency_confirmation_handler(callback_query: CallbackQuery, state: FSMContext) -> None:
    is_user_logged = await session_storage_service.is_user_logged(state)
    if is_user_logged:
        match = re.match(REGEX, callback_query.data)
        currency = match.group(1)
        await callback_query.message.answer(
            f"⚠️ Are you sure you want to remove {html.bold(currency)} from your tracked crypto currencies?",
            reply_markup=keyboards_builder.get_yes_no_keyboard(
                yes_button_callback_data=f"perform_remove_tracked_crypto_currency_$_{currency}"
            ),
        )
    else:
        await callback_query.message.answer(
